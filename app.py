...
from allennlp.interpret.saliency_interpreters import SaliencyInterpreter, \
    SimpleGradient, IntegratedGradient, SmoothGradient
...
def main(demo_dir: str,
    ...
    # This will be ``None`` if all the relevant environment variables are not
    # defined or if there is an exception when connecting to the database.
    demo_db = PostgresDemoDatabase.from_environment()
    if demo_db is None:
        logger.warning(
            "database credentials not provided, so not using database "
            "(permalinks disabled)")

    app = make_app(build_dir=f"{demo_dir}/build",
                   demo_db=demo_db, models=models)
    CORS(app)
    ...
    logger.info(
        "Server started on port %i.  Please visit: http://localhost:%i",
        port, port)
...
def make_app(build_dir: str,
        ...
        if demo_model is not None:
            ...
            if name in supported_interpret_models:
                app.interpreters[name]['simple_gradient'] = SimpleGradient(
                    predictor)
                app.interpreters[name]['integrated_gradient'] = \
                    IntegratedGradient(predictor)
                ...
                if name == 'masked-lm':
                    ...
                    # The SNLI model only has ELMo embeddings,
                    # which don't work with hotflip on their own.
                    continue
                else:
    ...
    def _caching_prediction(model: Predictor, data: str) -> JsonDict:
        """
        Just a wrapper around ``model.predict_json`` that allows us
        to use a cache decorator.
        """
    ...
    def _caching_interpret(interpreter: SaliencyInterpreter,
                           data: str) -> JsonDict:
        """
        Just a wrapper around ``model.interpret_from_json`` that allows us to
    ...
    def _caching_attack(attacker: Attacker, data: str,
                        ...
                        target: str) -> JsonDict:
        """
        Just a wrapper around ``model.attack_from_json`` that allows us to
        use a cache decorator.
        """
    ...
    def permadata(model_name: str) -> Response:
        """
        If the user requests a permalink, the front end will POST here with
        the payload
            { slug: slug }
        which we convert to an integer id and use to retrieve saved results
        from the database.
        """
    ...
    def predict(model_name: str) -> Response:
        ...
        record_to_database = request.args.get(
            "record", "true").lower() != "false"
        ...
        if model is None:
            raise ServerError("unknown model: {}".format(
                model_name), status_code=400)
        ...
        logger.info("request: %s", json.dumps(
            {"model": model_name, "inputs": data}))

        log_blob = {"model": model_name, "inputs": data,
                    "cached": False, "outputs": {}}

        # Record the number of cache hits before we hit the cache so we can
        # tell whether we hit or not.
        # In theory this could result in false positives.
        pre_hits = _caching_prediction.cache_info(
        ).hits  # pylint: disable=no-value-for-parameter
        ...
        if record_to_database and demo_db is not None:
            try:
                ...
                perma_id = demo_db.insert_request(
                    ...
                    model_name=model_name, inputs=data)
            except Exception:
                # TODO(joelgrus): catch more specific errors
                logger.exception(
                    "Unable to add request to database", exc_info=True)
        ...
        if record_to_database and demo_db is not None and perma_id is not None:
            try:
                ...
                # TODO(joelgrus): catch more specific errors
                logger.exception(
                    "Unable to add response to database", exc_info=True)
    ...
    def attack(model_name: str) -> Response:
        ...
        if attacker is None:
            raise ServerError("unknown attacker for model: {} {}".format(
                attacker_name, model_name), status_code=400)
        ...
        pre_hits = _caching_attack.cache_info().hits

        if use_cache and attack_cache_size > 0:
            # lru_cache insists that all function arguments be hashable,
            # so unfortunately we have to stringify the data.
            attack = _caching_attack(attacker, json.dumps(
                data), input_field_to_attack, grad_input_field,
                json.dumps(target))

        else:
            # if cache_size is 0, skip caching altogether
            attack = attacker.attack_from_json(
                inputs=data, input_field_to_attack=input_field_to_attack,
                grad_input_field=grad_input_field, target=target)

        post_hits = _caching_attack.cache_info().hits
    ...
    def interpret(model_name: str) -> Response:
        ...
        pre_hits = _caching_interpret.cache_info().hits
    ...
    def info() -> Response:
        ...
        return jsonify({
            ...
            "githubUrl": f"http://github.com/allenai/allennlp-demo/commit/{git_version}"})  # noqa

    @app.route('/health')
    def health() -> Response:  # pylint: disable=unused-variable
        return "healthy"

    # As an SPA, we need to return index.html for /model-name and
    # /model-name/permalink

    def return_page(permalink: str = None) -> Response:
        """return the page"""
        return send_file(os.path.join(build_dir, 'index.html'))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--demo-dir', type=str, default='demo/',
        help="directory where the demo HTML is located")
    parser.add_argument(
        '--cache-size', type=int, default=128,
        help="how many results to keep in memory")
    parser.add_argument(
        '--interpret-cache-size', type=int, default=500,
        help="how many interpretation results to keep in memory")
    parser.add_argument(
        '--attack-cache-size', type=int, default=500,
        help="how many attack results to keep in memory")
    parser.add_argument(
        '--models-file', type=str, default='models.json',
    ...
    models_group.add_argument(
        ...
        default=[], help='if specified, only load these models')
    models_group.add_argument(
        '--no-models', dest='no_models',
        action='store_true', help='start just the front-end with no models')

    args = parser.parse_args()

    models = load_demo_models(
        args.models_file, args.model, model_names_only=args.no_models)
