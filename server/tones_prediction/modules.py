...
def positional_encoding(inputs,
    ...
    Returns:
        A 'Tensor' with one more rank than inputs's, with the dimensionality
        should be 'num_units'
    '''
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        position_enc = np.array([
            [pos / np.power(10000, 2. * i / num_units) for i in range(
                num_units)] for pos in range(T)])
...
def multihead_attention(queries,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs = tf.layers.dropout(
            outputs, rate=dropout_rate, training=tf.convert_to_tensor(
                is_training))
...
def label_smoothing(inputs, epsilon=0.1):
    ...
    Args:
      inputs: A 3d tensor with shape of [N, T, V], where V is the number of
      vocabulary.
