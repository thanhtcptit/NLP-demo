...
def basic_tokenizer(sentence):
    ...
    for space_separated_fragment in sentence.strip().split():
        ...
        # return [w.lower() for w in words if w not in stop_words and w != ''
        # and w != ' ']
...
def load_train_data(train_path, vocab_path, maxlen):
    sents = [basic_tokenizer(line) for line in codecs.open(
        train_path, 'r', 'utf-8').read().split("\n")
        if line and line[0] != "<"]
    ...
    for idx, sent in tqdm(enumerate(sents)):
        sent = np.asarray([word2idx.get(w, unk_idx)
                           for w in sent[:maxlen + 1]])
        result[idx][0] = sos_idx
        result[idx][1:] = np.lib.pad(
            sent, [0, maxlen + 1 - len(sent)], 'constant',
