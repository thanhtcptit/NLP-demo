...
(GPT, GPT-2, BERT, RoBERTa). GPT and GPT-2 are fine-tuned using a causal
language modeling (CLM) loss while BERT and RoBERTa are fine-tuned
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", block_size=512):
        ...
        cached_features_file = os.path.join(
            directory, args.model_name_or_path +
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            # you should loook for a bigger one :-) and second you
...
def _rotate_checkpoints(args, checkpoint_prefix, use_mtime=False):
    ...
    for checkpoint in checkpoints_to_be_deleted:
        logger.info(
            "Deleting older checkpoint [{}] due to "
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm \
...
def main():
    ...
    parser.add_argument(
        "--mlm_probability", type=float, default=0.15, help="Ratio of tokens "
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Optional directory to store the pre-trained models downloaded "
    ...
    if args.model_type in ["bert", "roberta", "distilbert", "camembert"] and \
            not args.mlm:
        raise ValueError(
            "BERT and RoBERTa do not have LM heads but masked LM heads. "
    ...
    if args.eval_data_file is None and args.do_eval:
        raise ValueError(
            ...
            "Either supply a file to --eval_data_file or remove the "
            "--do_eval argument."
        )
    ...
    if args.local_rank == 0:
        # End of barrier to make sure only the first process in distributed
