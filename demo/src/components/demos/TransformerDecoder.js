...
const apiUrl = () => `${API_ROOT}/predict/transformer_decoder`
const apiUrlInterpret = () => `${API_ROOT}/interpret/transformer_decoder`
const apiUrlAttack = () => `${API_ROOT}/attack/transformer_decoder`

const NAME_OF_INPUT_TO_ATTACK = "tokens"
const NAME_OF_GRAD_INPUT = "grad_input_1"

const title = "Transformer Decoder with Beam Search for Vietnamese Tone Prediction";
...
const DEFAULT = "Anh la ong ngoai cua em";
...
const DEFAULT_MODEL = "Transformer Decoder"

const description = (
  <span>
Predict tones from Vietnamese text. <ExternalLink href="https://github.com/suicao/Vn-Accent-Restorer" target="_blank" rel="noopener">Ref</ExternalLink>
  </span>
)
...
const Choices = ({output, index, logits, words}) => {
  ...
  const lis = words[index].map((word, idx) => {
    ...
    return (
      <ListItem key={`${idx}-${cleanWord}`}><br></br>
        <ChoiceItem>
          <Token>{` Beam ${idx}: `} {cleanWord}</Token>
        </ChoiceItem>
      </ListItem>
    )
  })
