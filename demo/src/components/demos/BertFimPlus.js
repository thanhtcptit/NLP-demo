...
const apiUrl = () => `${API_ROOT}/predict/bert-fimplus`
const apiUrlInterpret = () => `${API_ROOT}/interpret/bert-fimplus`
const apiUrlAttack = () => `${API_ROOT}/attack/bert-fimplus`
const apiRandTitle = () => `${API_ROOT}/rand`

const NAME_OF_INPUT_TO_ATTACK = "tokens"
const NAME_OF_GRAD_INPUT = "grad_input_1"
const title = "Finetuned Bert for FimPlus";
...
const Choices = ({output, index, logits, words, choose, probabilities}) => {
  ...
  const lis = words[index].map((word, idx) => {
    const prob = probabilities[index][idx].toFixed(1)
